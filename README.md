...
A tray app that answers one question. Hold **F8** and a pill drops in with
two rows — your five-hour session and your seven-day week — each with the
real percentage spent, the tokens behind it, and how long until it resets.
Above them, whether Claude itself is up. Let go and it melts away.

Tap the key instead of holding it and the pill stays pinned. Either way it
floats over whatever you're doing, ignores your clicks, and leaves nothing
behind.

**You'll want [Claude Code](https://claude.com/claude-code) on the same
machine** — on a Pro or Max plan — because that's where the numbers come
from. Without it you still get Claude's live status. Nothing about your
account leaves your computer: the usage is read off this machine, and the
only things vibecheck asks the internet are how Claude is doing and whether
there's a newer vibecheck.
...
with no window: press the key and it's there. Flip on **Start at login** in
Settings and you can forget about it.
...
Everything else lives in the tray icon's menu: the two figures, show/hide,
settings, quit. The icon is the spark from the pill, in the colour the pill
would give the same news — green while Claude is fine, amber, orange, red as
...
`statusLine` in `~/.claude/settings.json` at a small shim that copies them to
a local file and then hands the line straight through to whatever status line
you already had. Being upfront, since it edits a Claude Code setting by
itself:
...
- `vibecheck --restore-statusline` puts yours back, whenever you like
...
**Without the shim the numbers keep working**, counted from Claude Code's
...
Quit from the tray menu, then `npm uninstall -g vibecheck-app` — or the usual
uninstaller for your platform. Run `vibecheck --restore-statusline` before
that if you want your old Claude Code status line back; a vibecheck that's
still installed puts the shim back on its next launch.
